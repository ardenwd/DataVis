...
// **** Your JavaScript code goes here ****
    ...
    byCompany.forEach(function(d,i){
        ...
        cGroup.append('g').call(xGrid).attr('class', 'x grid');
        cGroup.append('g').call(yGrid).attr('class', 'y grid');
        ...
        var cLabelText = cGroup.append("text");
        
        cLabelText.text(i).attr('class','company-label').style('fill',colorScale(i)).attr('transform', function(){
             return 'translate(' + trellisWidth /2 + ', ' + trellisHeight/2 + ')'});
        
        var xLabelText = cGroup.append("text");
        xLabelText.text("Date (by Month)").attr('class', 'x axis-label')
            .attr('transform', function(){ return 'translate('+ trellisWidth /2 + ', ' + (trellisHeight + 34)+ ')'});

        var yLabelText = cGroup.append("text");
        yLabelText.text("Stock Price (USD)").attr('class', 'y axis-label')
            .attr('transform', function(){ return 'translate( -35, 120) rotate(-90)'});
           // d3.group(d))
